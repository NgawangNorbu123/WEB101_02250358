const WEATHER_API_KEY = '3313f45616d37f8344d34a6a836473d0';
...
async function saveLocation() {
    ...
    try {
        ...
        try {
            ...
            console.error('POST response is not valid JSON:', text);
            throw new Error('POST response is not valid JSON');
        }
...
async function fetchSavedLocations() {
    try {
        ...
        savedLocations = data.slice(0, 5).map(item => {
            ...
            try {
                if (item.body && item.body.trim().startsWith('{')) {
...
async function updateLocation() {
    ...
    try {
        ...
        displayResponseInfo('PUT', `${PLACEHOLDER_API_URL}/${id}`, response.status, {
            sentData: locationData,
            serverResponse: text || 'No response body'
        });

    } catch (error) {
        console.log('PUT request issue:', error);
    }
